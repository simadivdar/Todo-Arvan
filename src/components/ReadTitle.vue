...
<template>
  <section class="flex justify-center">
    <div class="w-96 grid grid-cols-1 divide-y rounded-md p-6 bg-white shadow-lg">
      <div v-for="todo of todos" :key="todo.id" class="mb-[0.125rem] block min-h-[1.5rem] p-4">
        <input
          ...
          />
        <label
          class="inline-block font-normal pl-[0.15rem] hover:cursor-pointer"
          for="checkboxChecked">
          {{ todo.title }}
        </label>
      </div>
